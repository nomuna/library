...
export class BorrowedBookService {
  ...
  public resourceSearchUrl = SERVER_API_URL + 'api/_search/borrowed-books';
  ...
  search(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IBorrowedBook[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient(borrowedBook: IBorrowedBook): IBorrowedBook {
    const copy: IBorrowedBook = Object.assign({}, borrowedBook, {
      borrowDate: borrowedBook.borrowDate != null && borrowedBook.borrowDate.isValid() ? borrowedBook.borrowDate.format(DATE_FORMAT) : null
    });
    return copy;
  }
