...
export class BookComponentsPage {
  ...
  deleteButtons = element.all(by.css('lbr-book div table .btn-danger'));
  title = element.all(by.css('lbr-book div h2#page-heading span')).first();
...
export class BookUpdatePage {
  pageTitle = element(by.id('lbr-book-heading'));
...
export class BookDeleteDialog {
  private dialogTitle = element(by.id('lbr-delete-book-heading'));
  private confirmButton = element(by.id('lbr-confirm-delete-book'));
