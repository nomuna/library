...
describe('Author e2e test', () => {
  ...
  it('should load Authors', async () => {
    ...
    expect(await authorComponentsPage.getTitle()).to.eq('libraryApp.author.home.title');
  });
  ...
  it('should load create Author page', async () => {
    ...
    expect(await authorUpdatePage.getPageTitle()).to.eq('libraryApp.author.home.createOrEditLabel');
  ...
  it('should delete last Author', async () => {
    ...
    expect(await authorDeleteDialog.getDialogTitle()).to.eq('libraryApp.author.delete.question');
